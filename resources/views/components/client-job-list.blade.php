...
@endphp
<div class="overflow-x-auto w-full" x-data="{ contracts: '', hideAlreadyEvaluated: $persist(false) {{ $involvedGroupNames->count() > 0 ? ',' . $involvedGroupNames->map(fn($e) => 'showGroup_' . $e . ':$persist(true)')->join(',') : '' }} }">
    ...
    <table class="table w-full">
        ...
        <thead>
            ...
            <tr x-show="$store.empty">
                <td class="pl-0">
                    <div class="text-center italic text-xl rounded-lg bg-base-200 w-48">{{ __('Nothing to show') }}
                    </div>
                </td>
            </tr>
        </thead>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                ...
                // timeout is to let alpinejs events fire up before taking a decision...
            });
            ...
            function updateProjectsVisbility() {
                setTimeout(() => {
                    ...
                    @foreach ($jobs as $job)
                        atLeastOneProjectVisible |= toggleProjectVisibility('{{ $job->id }}');
                    @endforeach
        ...
        <tbody>
            @foreach ($jobs as $job)
                ...
                <tr x-show="$store.show{{ $job->id }} && $store.show{{ $job->id }}main" x-transition.opacity
                    x-data="{ massAction: false }">
                    <td colspan="6">
                        ...
                        <table :class="{ 'table-zebra': {!! $showGroupFilterJsEval !!} && !hideAlreadyEvaluated }"
                                                ...
                                                <dialog id="addContract{{ $job->id }}" class="modal">
                                                    <div class="modal-box">
                                                        ...
                                                        <form method="post" action="{{ route('contracts.store') }}"
                                                            ...
                                                            <label>
                                                                <input name="worker" class="input w-full"
                                                                    type="text"
                                                                    placeholder="{{ __('Select worker') }}"
                                                                    list="workers-list" />
                                                            </label>
                                                            ...
                                                            <input type="hidden" name="job_definition_id"
                                                                value="{{ $job->id }}" />
                                                        ...
                                                        <div class="modal-action">
                                                            ...
                                                            <button class="btn btn-success"
                                                                ...
                                                                <span id="SaveContractButton{{ $job->id }}"
                                                                    class="hidden"></span>
                                                                {{ __('Add') }}
                                                            </button>
                                                            ...
                                                            <form method="dialog">
                                                                ...
                                                                <button
                                                                    class="btn btn-error">{{ __('Cancel') }}</button>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </dialog>
                            ...
                            <tbody>
                                <form method="post" action="{{ route('contracts.destroyAll') }}"
                                    id="job-{{ $job->id }}-form" x-on:submit.prevent>
                                    @method('DELETE')
                                    @csrf
                                    <input type="hidden" name="job_id" value="{{ $job->id }}">
                                    @if (app()->environment('testing'))
                                        <input class="hidden" type="submit"
                                            id="job-{{ $job->id }}-form-input-for-test">
                                    @endif
                                    {{-- CONTRACTS HEADERS --}}
                                    <tr>
                                        <td>
                ...
                <div class="modal">
                    <div class="modal-box">
                        <h3 class="font-bold text-lg">
                            {{ __('Do you really want to delete the following contracts ?') }}</h3>
                        <p class="py-4">

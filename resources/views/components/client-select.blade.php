...
@php
    ...
    $providers = Cache::rememberForever('providers-' . $jobDefinition->id, function () use ($jobDefinition) {
        return $jobDefinition->getProviders();
    });

    //If a user is deleted/added, it will be reflected only 5 minutes later...
    $clients = Cache::rememberForever('clients-' . $jobDefinition->id, function () use ($jobDefinition, $providers) {
        return $jobDefinition->getClients($providers);
    });
@endphp
        ...
        <select class="select select-bordered {{ $size2 }}" name="{{ $name ?? 'client-' . $part->id }}">
            ...
            @foreach ($providers as $client)
                <option value="{{ $client->id }}"
                    {{ old('client') == $client->id || $selected == $client->id ? 'selected="selected"' : '' }}>
                    {{ $client->firstname . ' ' . $client->lastname }}
                    ({{ $client->getClientLoad(\App\Models\AcademicPeriod::current())['percentage'] }}%)
                </option>
            @endforeach
